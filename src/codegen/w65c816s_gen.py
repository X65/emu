...
def enc_addr(op, addr_mode, mem_access):
    if addr_mode == A_IMP or addr_mode == A_ACC or addr_mode == A_STC:
        ...
        op.t('_VDA(0);_SA(_E(c)?((c->AD+_X(c)+1)&0xFF):c->D+c->AD+_X16(c))+1;c->AD=_GD();')
        op.t('_VDA(c->DBR);_SA((_GD()<<8)|c->AD);')
...
def i_tax(o):
    cmt(o,'TAX')
    o.t('if(_i8(c)){_X(c)=_A(c);_NZ(_X(c));}else{_X16(c)=_C(c);_NZ16(_X16(c));}')

#-------------------------------------------------------------------------------
def i_tay(o):
    cmt(o,'TAY')
    o.t('if(_i8(c)){_Y(c)=_A(c);_NZ(_Y(c));}else{_Y16(c)=_C(c);_NZ16(_Y16(c));}')

#-------------------------------------------------------------------------------
def i_txa(o):
    cmt(o,'TXA')
    o.t('if(_a8(c)){_A(c)=_X(c);_NZ(_A(c));}else{_C(c)=_X16(c);_NZ16(_C(c));}')

#-------------------------------------------------------------------------------
def i_txy(o):
    cmt(o,'TXY')
    o.t('if(_i8(c)){_Y(c)=_X(c);_NZ(_Y(c));}else{_Y16(c)=_X16(c);_NZ16(_Y16(c));}')

#-------------------------------------------------------------------------------
def i_tya(o):
    cmt(o,'TYA')
    o.t('if(_a8(c)){_A(c)=_Y(c);_NZ(_A(c));}else{_C(c)=_Y16(c);_NZ16(_C(c));}')

#-------------------------------------------------------------------------------
def i_tyx(o):
    cmt(o,'TYX')
    o.t('if(_i8(c)){_X(c)=_Y(c);_NZ(_X(c));}else{_X16(c)=_Y16(c);_NZ16(_X16(c));}')
...
def i_tsx(o):
    cmt(o,'TSX')
    o.t('if(_i8(c)){_X(c)=_S(c);_NZ(_X(c));}else{_X16(c)=_S(c);_NZ16(_X16(c));}')
...
def i_pha(o):
    cmt(o,'PHA')
    o.t('_VDA(0);_SAD(0x0100|_S(c)--,(_a8(c)?_A(c):_B(c)));_WR();')
    o.t('if(_a8(c)){_FETCH();}else{_VDA(0);_SAD(0x0100|_S(c)--,_A(c));_WR();}')
...
def i_pla(o):
    ...
    o.t('_A(c)=_GD();if(_a8(c)){_NZ(_A(c));_FETCH();}else{_VDA(0);_SA(0x0100|++_S(c));}')
    o.t('_B(c)=_GD();_NZ16(_C(c));')

#-------------------------------------------------------------------------------
def i_phx(o):
    cmt(o,'PHX')
    o.t('_VDA(0);_SAD(0x0100|_S(c)--,(_i8(c)?_X(c):(_X16(c)>>8)));_WR();')
    o.t('if(_i8(c)){_FETCH();}else{_VDA(0);_SAD(0x0100|_S(c)--,_X(c));_WR();}')
...
def i_plx(o):
    ...
    o.t('_SA(c->PC);') # second junk read from current PC
    o.t('_VDA(0);_SA(0x0100|++_S(c));')   # read actual byte
    o.t('_X(c)=_GD();if(_i8(c)){_NZ(_X(c));_FETCH();}else{_VDA(0);_SA(0x0100|++_S(c));}')
    o.t('_X16(c)=_X(c)|(_GD()<<8);_NZ16(_X16(c));')

#-------------------------------------------------------------------------------
def i_phy(o):
    cmt(o,'PHY')
    o.t('_VDA(0);_SAD(0x0100|_S(c)--,(_i8(c)?_Y(c):(_Y16(c)>>8)));_WR();')
    o.t('if(_i8(c)){_FETCH();}else{_VDA(0);_SAD(0x0100|_S(c)--,_Y(c));_WR();}')
...
def i_ply(o):
    ...
    o.t('_SA(c->PC);') # second junk read from current PC
    o.t('_VDA(0);_SA(0x0100|++_S(c));')   # read actual byte
    o.t('_Y(c)=_GD();if(_i8(c)){_NZ(_Y(c));_FETCH();}else{_VDA(0);_SA(0x0100|++_S(c));}')
    o.t('_Y16(c)=_Y(c)|(_GD()<<8);_NZ16(_Y16(c));')
...
def i_jmpx(o):
    ...
    o.t('_VPA();_SA(c->PC++);c->AD=_GD();')
    o.t('_SA(c->PC);c->AD=(_GD()<<8)|c->AD;')
    o.t('_VDA(c->DBR);c->AD|=_GD()<<8;_SA((c->AD&0xFF00)|((c->AD+_X(c))&0xFF));')
    o.t('_VDA(c->DBR);_SA((c->AD&0xFF00)|((c->AD+_X(c)+1)&0xFF));c->AD=_GD();')
    o.t('c->PC=(_GD()<<8)|c->AD;')
...
def i_jsrx(o):
    ...
    o.t('_VDA(c->DBR);_SA((c->AD&0xFF00)|((c->AD+_X(c))&0xFF));')
    o.t('_VDA(c->DBR);_SA((c->AD&0xFF00)|((c->AD+_X(c)+1)&0xFF));c->AD=_GD();')
    o.t('c->PC=(_GD()<<8)|c->AD;')
...
def i_jsl(o):
    ...
    o.t('_SA(0x0100|_S(c)--);c->AD=(_GD()<<8)|c->AD;')
    # read bank of target address
    o.t('_VPA();_SA(c->PC++);')
    # write PC high byte to stack
    o.t('_VDA(0);_SAD(0x0100|_S(c)--,c->PC>>8);c->PBR=_GD();_WR();')
    # write PC low byte to stack
    o.t('_VDA(0);_SAD(0x0100|_S(c)--,c->PC);_WR();')
    # load PC and done
    o.t('c->PC=c->AD;')
...
def i_rts(o):
    ...
    o.t('c->PC=(_GD()<<8)|c->AD;_SA(0x0100|_S(c));')
...
def i_rtl(o):
    ...
    o.t('_VDA(0);_SA(0x0100|++_S(c));c->PC=(_GD()<<8)|c->AD;')
    # next tick is opcode fetch
    o.t('c->PBR=_GD();');
...
def i_rti(o):
    ...
    o.t('c->PC=(_GD()<<8)|c->AD;if(_E(c)){_FETCH();}else{_VDA(0);_SA(0x0100|++_S(c));}')
...
def i_dex(o):
    cmt(o,'DEX')
    o.t('if(_i8(c)){_X(c)--;_NZ(_X(c));}else{_X16(c)--;_NZ16(_X16(c));}')

#-------------------------------------------------------------------------------
def i_dey(o):
    cmt(o,'DEY')
    o.t('if(_i8(c)){_Y(c)--;_NZ(_Y(c));}else{_Y16(c)--;_NZ16(_Y16(c));}')
