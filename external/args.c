...
static void exit_with_error(bool should_exit, const char* format_string, ...) {
...
static int try_str_to_int(bool should_exit, const char* string) {
    char *endptr;
    errno = 0;
    long result = strtol(string, &endptr, 0);
    if (errno == ERANGE || result > INT_MAX || result < INT_MIN) {
        exit_with_error(should_exit, "'%s' is out of range", string);
    }
    if (*endptr != '\0') {
        exit_with_error(should_exit, "cannot parse '%s' as an integer", string);
    }
    return (int) result;
}
...
static double try_str_to_double(bool should_exit, const char* string) {
    char *endptr;
    errno = 0;
    ...
    if (*endptr != '\0') {
        exit_with_error(should_exit, "cannot parse '%s' as a floating-point value", string);
    }
    return result;
}
...
static bool option_try_set(bool should_exit, Option* opt, char* arg) {
    if (opt->type == OPT_STR) {
        return option_append_value(opt, (OptionValue){.str_val = arg});
    }
    else if (opt->type == OPT_INT) {
        int value = try_str_to_int(should_exit, arg);
        return option_append_value(opt, (OptionValue){.int_val = value});
    }
    else if (opt->type == OPT_DBL) {
        double value = try_str_to_double(should_exit, arg);
...
void ap_set_failing(ArgParser* parser, bool failing) {
    parser->failing = failing;
}
...
static Option* ap_get_opt(ArgParser* parser, const char* name) {
    ...
    if (!map_get(parser->option_map, name, &opt)) {
        exit_with_error(parser->failing, "'%s' is not a registered flag or option name", name);
    }
    return (Option*)opt;
}
...
int* ap_get_args_as_ints(ArgParser* parser) {
    ...
    for (int i = 0; i < count; i++) {
        *(args + i) = try_str_to_int(parser->failing, parser->positional_args->entries[i]);
    }
    return args;
}
...
double* ap_get_args_as_doubles(ArgParser* parser) {
    ...
    for (int i = 0; i < count; i++) {
        *(args + i) = try_str_to_double(parser->failing, parser->positional_args->entries[i]);
    }
    return args;
}
...
ArgParser* ap_new_cmd(ArgParser* parent_parser, const char* name) {
    ...
    cmd_parser->failing = parent_parser->failing;
...
static void ap_handle_equals_opt(ArgParser* parser, const char* prefix, const char* arg, ArgStream* stream) {
    ...
    *strchr(array, '=') = '\0';
    ...
    if (!found) {
        free(array);
        exit_with_error(parser->failing, "%s%s is not a recognised option name", prefix, name);
    }

    if (option->type == OPT_FLAG) {
        free(array);
        exit_with_error(parser->failing, "flag %s%s does not accept an argument", prefix, name);
    }

    if (strlen(value) == 0) {
        free(array);
        exit_with_error(parser->failing, "missing argument for %s%s", prefix, name);
    }

    if (!option_try_set(parser->failing, option, value)) {
        ap_set_memory_error_flag(parser);
    }

    if (option->is_greedy) {
        while (argstream_has_next(stream)) {
            if (!option_try_set(parser->failing, option, argstream_next(stream))) {
                ap_set_memory_error_flag(parser);
            }
        }
    }
...
static void ap_handle_long_opt(ArgParser* parser, const char* arg, ArgStream* stream) {
    ...
    if (map_get(parser->option_map, arg, (void**)&option)) {
        ...
        exit_with_error(parser->failing, "missing argument for --%s", arg);
    }

    if (strcmp(arg, "help") == 0 && parser->helptext != NULL) {
        puts(parser->helptext);
        if (parser->failing) exit(0); else return;
    }
    ...
    exit_with_error(parser->failing, "--%s is not a recognised flag or option name", arg);
}
...
static void ap_handle_short_opt(ArgParser* parser, const char* arg, ArgStream* stream) {
    for (size_t i = 0; i < strlen(arg); i++) {
        ...
        if (!found) {
            ...
            if (strlen(arg) > 1) {
                exit_with_error(parser->failing, "'%c' in -%s is not a recognised flag or option name", arg[i], arg);
            }
            exit_with_error(parser->failing, "-%s is not a recognised flag or option name", arg);
        }
        ...
        if (strlen(arg) > 1) {
            exit_with_error(parser->failing, "missing argument for '%c' in -%s", arg[i], arg);
        }
        ...
        // Is the argument the automatic 'help' command?
            ...
            if (argstream_has_next(stream)) {
                ...
                if (map_get(parser->command_map, name, (void**)&cmd_parser)) {
                    ...
                    exit_with_error(parser->failing, "'%s' is not a recognised command", name);
                }
            } else {
                exit_with_error(parser->failing, "the 'help' command requires an argument");
            }
        }
